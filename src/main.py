...
def search_restaurant(update, context):
    ...
    gh_api = "https://api-pp.grubhub.com/"
    query = update.message.text

    results = requests.get(f"{gh_api}/search?sort=relevancy&q={query}").json()["results"]
    button_list = []
    rest_name = None
    if results:
...
def checker_query_handler(update, context):
    gh_api = "api-pp.grubhub.com/restaurants/availability_summaries?ids=11402556&amp;ids=296460"
    ...
    result = requests.get(f"{gh_api}/{slug}").json()["results"][0]
...
Thank you for using GrubHub Checker Bot :)\n\
...
def register_handling(update, context):
    ...
    if action == "NO":
        query.edit_message_text("Thank you for using GrubHub Checker Bot :)\n\n\
To re-run the bot, please write /start")
...
def cancel(update, context):
    try:
        update.message.reply_text("Thank you for using GrubHub Checker Bot :)\n\n\
To re-run the bot, please write /start")
    except:
        pass
    return ConversationHandler.END
